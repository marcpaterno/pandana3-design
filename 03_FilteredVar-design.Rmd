...
Each case comes with a textual description (generally brief), and a table showing for the `Var` and `Cut` data member of the `FilteredVar`, and d an example of the list of index columns that `Var` has.
...
Case with var being a FilteredVar: var is electron pt, phi for electrons in events with met >10; we want electron quality for such electrons with pt > 15.  
...
| var  | a  b          | electron quality q1, q2                 |
...
|------|---------------|-----------------------------------------|
| var  | a             | events met                              |
| cut  | a  b          | electrons with quality q1 > 0.75        |
